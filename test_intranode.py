...
def checkpoint_hold(group: dist.ProcessGroup, local_rank: int, rank: int, seconds: int, label: str):
    if seconds <= 0:
        return

    torch.cuda.synchronize()
    group.barrier()
    print(f'[checkpoint] rank={rank} pid={os.getpid()} holding for {seconds}s at {label}', flush=True)
    time.sleep(seconds)
    group.barrier()
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    checkpoint_hold(group, local_rank, rank, args.hold_secs, 'post-test-pre-destroy')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hold-secs', type=int, default=0,
                        help='Hold all ranks alive after the test and before teardown for checkpointing')
